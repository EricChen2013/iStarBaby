...
public class IOTCAPIs {
	...
	/**
	 * IOTC servers have no response, probably caused by many types of Internet
	 * connection issues. See [Troubleshooting](..\Troubleshooting\index.htm#
	 * IOTC_ER_SERVER_NOT_RESPONSE)
	 */
	public static final int IOTC_ER_SERVER_NOT_RESPONSE = -1;

	/**
	 * IOTC masters cannot be resolved their domain name, probably caused by
	 * network connection or DNS setting issues. See
	 * [Troubleshooting](..\Troubleshooting
	 * \index.htm#IOTC_ER_FAIL_RESOLVE_HOSTNAME)
	 */
	...
	/**
	 * IOTC module fails to create threads. Please check if OS has ability to
	 * create threads for IOTC module.
	 */
	public static final int IOTC_ER_FAIL_CREATE_THREAD = -5;

	/**
	 * IOTC module fails to create sockets. Please check if OS supports socket
	 * service
	 */
	...
	/**
	 * The specified UID is not licensed. See
	 * [Troubleshooting](..\Troubleshooting\index.htm#IOTC_ER_UNLICENSE)
	 */
	...
	/**
	 * IOTC module fails to get the local IP address See
	 * [Troubleshooting](..\Troubleshooting\index.htm#IOTC_ER_FAIL_GET_LOCAL_IP)
	 */
	...
	/**
	 * The number of IOTC sessions has reached maximum. Please use
	 * IOTC_Set_Max_Session_Number() to set up the max number of IOTC sessions
	 */
	...
	/**
	 * The remote site already closes this IOTC session. Please call
	 * IOTC_Session_Close() to release IOTC session resource in locate site.
	 */
	...
	/**
	 * The client fails to connect to a device because the device is not
	 * listening for connections. See
	 * [Troubleshooting](..\Troubleshooting\index.
	 * htm#IOTC_ER_DEVICE_NOT_LISTENING)
	 */
	public static final int IOTC_ER_DEVICE_NOT_LISTENING = -24;

	/**
	 * The IOTC channel of specified channel ID is not turned on before
	 * transferring data.
	 */
	...
	/**
	 * Too few masters are specified when initializing IOTC module. Two masters
	 * are required for initialization at minimum.
	 */
	...
	/**
	 * The number of IOTC channels for a IOTC session has reached maximum, say,
	 * MAX_CHANNEL_NUMBER.
	 */
	...
	/**
	 * Cannot connect to IOTC servers in TCP See
	 * [Troubleshooting](..\Troubleshooting
	 * \index.htm#IOTC_ER_TCP_CONNECT_TO_SERVER_FAILED)
	 */
	public static final int IOTC_ER_TCP_CONNECT_TO_SERVER_FAILED = -33;

	/**
	 * A client wants to connect to a device in non-secure mode while that
	 * device supports secure mode only.
	 */
	public static final int IOTC_ER_CLIENT_NOT_SECURE_MODE = -34;

	/**
	 * A client wants to connect to a device in secure mode while that device
	 * does not support secure mode.
	 */
	 ...
	 * or IOTC_Connect_ByName2() is not valid. Please see IOTCConnectionMode for
	 * possible modes.
	 */
	...
	/**
	 * The specified device does not support advance function (TCP relay and
	 * P2PTunnel module)
	 */
	...
	/**
	 * A client fails to use UDP relay mode to connect to a device because UDP
	 * relay mode is not supported for that device by IOTC servers
	 */
	public static final int IOTC_ER_NOT_SUPPORT_RELAY = -43;

	/**
	 * No IOTC server information while device login or client connect because
	 * no IOTC server is running or not add IOTC server list
	 */
	...
	public native static int IOTC_Initialize(int UDPPort,
			String P2PHostNamePrimary, String P2PHostNameSecondary,
			String P2PHostNameThird, String P2PHostNameFourth);

	public native static int IOTC_Initialize2(int UDPPort);

	public native static int IOTC_DeInitialize();

	public native static int IOTC_Device_Login(String UID, String DeviceName,
			String DevicePWD);

	public native static int IOTC_Listen(long Timeout_ms);

	public native static int IOTC_Listen2(long Timeout_ms, String AesKey,
			int mode); // mode-> 0: equal IOTC_Listen(), 1: only accept secure
						// connection, 2: accept non-secure and secure then use
						// IOTC_Session_Check() to know which mode

	public native static int IOTC_Connect_ByUID(String UID);

	public native static int IOTC_Connect_ByUID2(String UID, String AesKey,
			int mode); // mode-> 0: equal IOTC_Listen(), 1: only accept secure
						// connection, 2: accept non-secure and secure then use
						// IOTC_Session_Check() to know which mode
	...
	public native static int IOTC_Session_Read(int SID, byte[] Buf,
			int Max_size, int Timeout_ms, int ChID);

	public native static int IOTC_Session_Write(int SID, byte[] Buf, int Size,
			int ChID);
	...
	public native static st_LanSearchInfo[] IOTC_Lan_Search(int[] arrNum,
			int nWaitTimeMs);

	public native static st_LanSearchInfo2[] IOTC_Lan_Search2(int[] arrNum,
			int nWaitTimeMs);
	...
	public native static void IOTC_Get_Login_Info_ByCallBackFn(Object obj); // Callback
																			// method,
																			// pass
																			// object
																			// itself.

	public native static int IOTC_Session_Check_ByCallBackFn(int SID, Object obj); // Callback
																					// method,
																					// pass
																					// object
																					// itself.

	public native static int IOTC_Set_Partial_Encryption(int SID,
			int bPartialEncryption);

	public native static int IOTC_Session_Read_Check_Lost(int SID, byte[] Buf,
			int Max_size, int Timeout_ms, int[] PacketSN, int[] FlagLost,
			int ChID);
	...
	public void sessionStatusCB(int nSID, int nErrCode) {
		// This is callback method, we can modify it.
		System.out.println("[parent] SessionStatus Callback, nSID=" + nSID
				+ ", nErrCode=" + nErrCode);
	};
