...
public class AVAPIs {
	...
	// --{{inner iotype-----------------------------------------------------
	public static final int IOTYPE_INNER_SND_DATA_DELAY = 0xFF; // C--->D:
																// avClient(AP)
																// change time
																// interval of
																// sending
																// packets by
																// avSendFrameData(avServer)
	...
	/**
	 * The number of AV channels has reached maximum. The maximum number of AV
	 * channels is determined by the passed-in argument of avInitialize()
	 */
	...
	/**
	 * AV fails to create threads. Please check if OS has ability to create
	 * threads for AV.
	 */
	public static final int AV_ER_FAIL_CREATE_THREAD = -20004;

	/**
	 * A warning error code to indicate that the sending queue of video frame of
	 * an AV server is almost full, probably caused by slow handling of an AV
	 * client or network issue. Please note that this is just a warning, the
	 * video frame is actually put in the queue.
	 */
	...
	/**
	 * The client fails in authentication due to incorrect view account or
	 * password
	 */
	...
	/**
	 * The remote site already closes the IOTC session. Please call
	 * IOTC_Session_Close() to release local IOTC session resource
	 */
	...
	/**
	 * The AV channel of specified AV channel ID is already in sending IO
	 * control process
	 */
	...
	public native static int avRecvIOCtrl(int avIndex, int[] pioType,
			byte[] ioCtrlBuf, int ioCtrlBufMaxSize, int timeout_ms);

	public native static int avSendIOCtrlExit(int avIndex);

	// Device Side
	public native static int avServStart(int nSID, byte[] viewAcc,
			byte[] viewPwd, long timeout_sec, long servType, int ChID);

	public native static void avServStop(int avIndex);

	public native static void avServExit(int nSID, int ChID);

	public native static int avSendFrameData(int avIndex, byte[] data,
			int dataSize, byte[] pFrmInfo, int FrmInfoSize);

	public native static int avSendAudioData(int avIndex, byte[] data,
			int dataSize, byte[] pFrmInfo, int FrmInfoSize);

	// Client Side
	public native static int avClientStart(int nSID, String viewAcc,
			String viewPwd, long timeout_sec, long[] pservType, int ChID);

	public native static int avClientStart2(int nSID, String viewAcc,
			String viewPwd, long timeout_sec, long[] pservType, int ChID,
			int[] bResend);
	...
	public native static int avRecvFrameData(int avIndex, byte[] buf,
			int bufMaxSize, byte[] pFrmInfo, int FrmInfoMaxSize, int[] pFrmNo);

	public native static int avRecvFrameData2(int avIndex, byte[] buf,
			int inBufSize, int[] outBufSize, int[] outFrmSize,
			byte[] pFrmInfoBuf, int inFrmInfoBufSize, int[] outFrmInfoBufSize,
			int[] pFrmNo);

	public native static int avRecvAudioData(int avIndex, byte[] buf,
			int bufMaxSize, byte[] pFrmInfo, int FrmInfoMaxSize, int[] pFrmNo);
	...
	public native static int avServStart2(int nSID, String viewAcc,
			String viewPwd, long timeout_sec, long servType, int ChID);

	public native static int avServStart3(int nSID, String viewAcc,
			String viewPwd, long timeout_sec, long servType, int ChID,
			int[] bResend);

	public native static void avServSetResendSize(int avIndex, long nSize);

	public native static int avServSetDelayInterval(int avIndex,
			int nPacketNum, int nDelayMs);
